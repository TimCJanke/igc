...
def get_mmdtest_matrix(samples_true, samples_models, sigma=-1, computeMMDs=False, standardize=False):
    
    model_names = list(samples_models.keys())
    
    pvalue_df = pd.DataFrame(index=model_names, columns=model_names)
    mmd_scores = {}

    for i,key_i in enumerate(model_names):
        for j,key_j in enumerate(model_names):
            if i!=j:
                pval,_,_,mmd_xy,_ = MMD_3_Sample_Test(X=samples_true, Y=samples_models[key_i], Z=samples_models[key_j], sigma=sigma, computeMMDs=computeMMDs)
                
                pvalue_df.loc[key_i,key_j] = pval
                mmd_scores[key_i] = mmd_xy
                
    return pvalue_df, mmd_scores
...
def MMD_3_Sample_Test(X,Y,Z,sigma=-1,SelectSigma=2,computeMMDs=False):
    ...
    X: test samples
    Y: sammples form model 1
    Z: samples from model 2
    
    '''
    n = np.min([X.shape[0], Y.shape[0], Z.shape[0]])
