...
for train_index, test_index in kf.split(pobs.values):
    ...
    # IGC model
    pobs_train_igc = pobs_train[0:int(np.floor(len(pobs_train)/100)*100)]
    cop_igc = ImplicitGenerativeCopula(dim_latent=pobs_train.shape[1]*4, dim_out=pobs_train.shape[1], n_samples_train=200, n_layers=2, n_neurons=100)        
